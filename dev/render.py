...
This is the core pipeline the local dev harness (server.py) and, later, the
real HA integration both use. Kept dependency-light and framework-free on
purpose so it's easy to port.
"""
...
if __name__ == "__main__":
    import sys
    src = sys.argv[1] if len(sys.argv) > 1 else "dev/_sample.h5"
    out = sys.argv[2] if len(sys.argv) > 2 else "dev/_sample_render.png"
    with open(out, "wb") as fh:
        fh.write(render_png_bytes(src))
    print(f"wrote {out}")
